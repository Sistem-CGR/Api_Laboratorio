...
let connect = (req, res) => {
  clientPLC
    .readHoldingRegisters(0, 7)
    .then((result) => {
      res.status(200).json(true);
    })
    .catch(function () {
      console.error(
        "Conectar " + require("util").inspect(arguments, { depth: null })
      );
...
let reconnect = (req, res) => {
  ...
  clientPLC
    ...
    .catch(function () {
      socket.connect(options);
      console.error(
        "Reconectar " + require("util").inspect(arguments, { depth: null })
      );
      res.status(202).json(false);
    });
};
...
let dataRegisters = (req, res) => {
  clientPLC
    ...
    .catch(function () {
      console.error(
        "dataRegisters" + require("util").inspect(arguments, { depth: null })
      );
      res.status(202).json(require("util").inspect(arguments, { depth: null }));
    });
};
...
app.get("/", (req, res) => {
  res.json({
    mensaje: "Api jsmodbus metodo GET :) - LAboratorio de pruebas CGR",
  });
});
app.post("/", (req, res) => {
  res.json({
    mensaje: "Api jsmodbus metodo POST :) - LAboratorio de pruebas CGR",
  });
});
